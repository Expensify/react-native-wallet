...
- 🔐 UAPP (User Account Payment Provisioning) support for enhanced security
...
- **Google Opaque Payment Card** (Google OPC) - an encrypted version of the OPC required for UAPP (User Account Payment Provisioning) flow. This is generated by your backend using Google's encryption keys during the provisioning process. [Learn more about UAPP →](./docs/UAPP.md)
...
# API Reference
The library offers functions for seamless integration and use of the Apple Wallet and Google Wallet APIs. Additionally, it includes listeners and handlers for managing provisioning events. Below, these functions are described along with the data types involved.
...
| **AndroidCardData** | Data related to a card that is to be added on Android platform wallets. | `network: string`,<br>`opaquePaymentCard: string`,<br>`cardHolderName: string`,<br>`lastDigits: string`,<br>`userAddress: UserAddress`,<br>`isVirtualCard?: boolean`,<br>`isBounceProvisioned?: boolean` |
...
| **AndroidPaymentCredentialsResponse** | Response object containing encrypted payment credentials for UAPP flow. [Learn more →](./docs/UAPP.md) | `opaquePaymentCard: string`,<br>`googleOpaquePaymentCard: string` |
| **onPaymentCredentialsRequestPayload** | Payload received when Google Wallet requests payment credentials during UAPP provisioning. [Learn more →](./docs/UAPP.md) | `requestId: string`,<br>`serverSessionId: string`,<br>`walletId: string`,<br>`opaquePaymentCard: string` |
...
The `AndroidCardData` type includes two optional boolean fields that provide additional information about the card being provisioned:

#### `isVirtualCard?: boolean`

Indicates whether the card being provisioned is a virtual card. Virtual cards are digitally-generated card numbers that are linked to a physical card account. They provide enhanced security by:
- Allowing users to make online purchases without exposing their physical card number
- Enabling easy cancellation and regeneration without affecting the physical card
- Providing transaction-specific or merchant-specific card numbers

When set to `true`, this flag informs Google Wallet that the card being provisioned is a virtual card, which may affect how the card is displayed and managed in the wallet interface.

#### `isBounceProvisioned?: boolean`

Indicates whether the card has been provisioned through "bounce provisioning." Bounce provisioning is a two-step provisioning flow where:
1. The user initially provisions a card directly through Google Wallet (outside your app)
2. Your app later "bounces" or redirects the user back to complete additional verification or setup steps

This flag helps Google Wallet track cards that were initially added through the Google Wallet app but require additional app-specific configuration or verification. When set to `true`, it signals that this card was previously provisioned through the Google Wallet interface and is now being completed or updated through your application.
...
For Android applications that require enhanced security for card provisioning, this library supports UAPP (User Account Payment Provisioning) flow with Google Wallet.

📖 **[Read the complete UAPP documentation →](./docs/UAPP.md)**

The UAPP guide covers:
- What UAPP is and how it works
...
> [!WARNING]
> **Testing is mandatory before production release.** Your access to the Google Wallet production environment may be revoked if you do not provide the required test evidence for launch approval. Complete all testing requirements before deploying your app.
...
The app will need to be reviewed by Google. During this process, it will need to pass 4 mandatory test cases that are specified in the [official Google testing documentation](https://developers.google.com/pay/issuers/apis/push-provisioning/android/test-cases). They verify how your app handles card state tracking in different scenarios.

**Key testing criteria include:**
- Proper handling of card states (active, pending, suspended, etc.)
- Correct UI behavior when cards are already provisioned
- Secure handling of sensitive payment data
- Compliance with Google's branding guidelines
- Proper error handling and user feedback

Please refer to the [complete testing criteria documentation](https://developers.google.com/pay/issuers/apis/push-provisioning/android/test-cases) before submitting your app for review.
